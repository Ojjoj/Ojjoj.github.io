...
function validateDocument() {
    ...
    // Perform validation
    checkValidity(url, name);
}

function checkValidity(url, name) {
    ...
    fetch("winners.txt")
        ...
        .then(data => {
            ...
            for (var i = 0; i < winners.length; i++){
                if(name == winners[i].trim()){
                    isValid = true;
                    break;
                }
            }
            // Display the result on the webpage
            displayResult(isValid, name); 
        })
        .catch(error => {
